                        ...
                        FileUpload::make('file')
                            ...
                            ->getUploadedFileNameForStorageUsing(
                                fn(TemporaryUploadedFile $file): string => (string)str($file->getClientOriginalName())
                                    ->prepend(now()->format('Y-m-d') . '-troca-serviço-' . str_replace(' ', '_', auth()->user()->name) . '-' . now()->format('h-i-s') . '-')
                            ),

                    ]),

                Section::make('Ciência do 2º aluno envolvido')
                    ->disabled(fn(string $operation, Get $get): bool => ($operation === 'edit' && trim($get('first_shift_paying_military')
                            ) !== trim(auth()->user()->name)) || $get('status') !== 'Em andamento')
                    ->hiddenOn('create')
